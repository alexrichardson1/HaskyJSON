...
testPrimitive =
  TestList
    [ TestCase (assertEqual "null" (Just (JObject [("x", JNull)], "")) (runParser jsonObject "{\"x\":null}")),
...
testArray =
  TestList
    [ TestCase (assertEqual "empty" (Just (JObject [("x", JArray [])], "")) (runParser jsonObject "{\"x\":[]}")),
...
testObject =
  TestList
    [ TestCase (assertEqual "empty" (Just (JObject [], "")) (runParser jsonObject "{}")),
